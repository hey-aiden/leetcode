...
/**
 * 763. 划分字母区间: 给你一个字符串 s 。我们要把这个字符串划分为尽可能多的片段
 *
 * 同一字母最多出现在一个片段中
 *
...
var partitionLabels = function (s) {
    /**
     * 关键是同一个字母，最多在一个片段中，意味着，如果包含字母a，那么这个片段就要一直包含掉所有的该字母
     *
     * 有点像合并区间呢
     */
    ...
    // 可能拿到的数据是 [0, 9], [1, 2], [3, 5]
    const gapList = []
    let prevEnd = null
    for (const [start, end] of strMap.values()) {
        if (prevEnd && prevEnd > start) {
            gapList[gapList.length - 1][1] = Math.max(prevEnd, end)
        } else {
            gapList.push([start, end])
        }
        prevEnd = Math.max(prevEnd, end)
    }

    return gapList.map((item) => {
...
var totalFruit = function (fruits) {
    ...
    while (right < len) {
        ...
        while (fruitMap.size > 2) {
            const delKey = fruits[left]
            const prev = fruitMap.get(delKey)
            if (prev === 1) {
                fruitMap.delete(delKey)
            } else {
                fruitMap.set(delKey, prev - 1)
            }
            left++
        }
        let total = 0
